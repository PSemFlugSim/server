			...
			while (Launcher.isRunning()) {
				if (positionProvider != null) {
					...
					if (position == null) {
						...
						armLengths[0] = (int) Math.round(((double) position.getLength1() / (double) FSBPosition.MAX)
								* (double) arms[0].getMaxPosition());
						armLengths[1] = (int) Math.round(((double) position.getLength2() / (double) FSBPosition.MAX)
								* (double) arms[1].getMaxPosition());
						armLengths[2] = (int) Math.round(((double) position.getLength3() / (double) FSBPosition.MAX)
								* (double) arms[2].getMaxPosition());
						armLengths[3] = (int) Math.round(((double) position.getLength4() / (double) FSBPosition.MAX)
								* (double) arms[3].getMaxPosition());
						armLengths[4] = (int) Math.round(((double) position.getLength5() / (double) FSBPosition.MAX)
								* (double) arms[4].getMaxPosition());
						armLengths[5] = (int) Math.round(((double) position.getLength6() / (double) FSBPosition.MAX)
								* (double) arms[5].getMaxPosition());
						...
						// loop until all arms are arrived at the desired
						// position
						boolean[] arrived = new boolean[ARM_AMOUNT];
						while (!allTrue(arrived)) {

							// check each arm's position if it reached its
							// desired position
							for (int i = 0; i < ARM_AMOUNT; i++) {
								if (arms[i] == null)
									continue;
								if (arms[i].getDirection() == 1) {
									// if currently moving forward, check if
									// position is greater/equal than desired
									// position
									if (arms[i].getPosition() >= armLengths[i]) {
										// then stop
										arms[i].stop();
									...
									// if currently moving backward, check if
									// position is smaller/equal than desired
									// position
									if (arms[i].getPosition() <= armLengths[i]) {
										// then stop
										arms[i].stop();
